...
// Function to handle Student Lists frame
function accessStudentLists() {
    const studentListsLink = data.links["student_lists"];
    window.open(studentListsLink, '_blank');
}
...
function accessWorkspace() {
    ...
    if (data.passcodes[passcode]) {
        if (passcode === "admin") {
            ...
            const workspaces = Object.keys(data.links).filter(key => key.startsWith('workspace_'));
            const results = workspaces.map(ws => 
                `<tr>
                    <td>${data.passcodes[passcode].name}</td>
                    <td><a href="${data.links[ws]}" target="_blank">Click to Access</a></td>
                </tr>`).join("");
            localStorage.setItem('results', results);
            localStorage.setItem('teacherName', data.passcodes[passcode].name);
            window.location.href = 'results.html';
        } else if (data.passcodes[passcode].workspace) {
            // Teacher is directed to their specific workspace
